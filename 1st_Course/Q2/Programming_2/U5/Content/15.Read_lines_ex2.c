...
int main(){
    // Array of strings containing passages from "Don Quixote"
    char *phrase[7] = {
        ...
        "His family consisted of a housekeeper somewhat advanced in years, a niece not quite twenty, and a lad for the field and market, who bore a resemblance to the master of the house in so far as he was not a little mischievous.",
        "The age of our gentleman bordered upon fifty years; he was of a robust constitution, spare, gaunt-featured, a very early riser and a great sportsman.",
    ...
    FILE *filePointer;
    filePointer = fopen("quijote_4.txt", "w");

    // Write the first line and print it
    fputs(phrase[0], filePointer);
    printf("First line only: %s \n", phrase[0]);

    // Return to the beginning of the file
    rewind(filePointer);

    // Write each line to the file and print it
    for (int i = 0; i < 7; i++) {
        fputs(phrase[i], filePointer);
        fputc('\n', filePointer); // Add newline character after each line
        printf("Line %d: %s \n", i + 1, phrase[i]);
    }
