...
} // Esta función recorre cada elemento de la matriz y lo imprime. Después de imprimir todos los elementos de una fila, imprime una nueva
línea.
    ...
    int
    esElementoValle(int matriz[][NUM_COLUMNAS], int n_filas, int n_columnas, int f, int c)
{
    ...
    for (int i = -1; i <= 1; i++)
    {
        ...
        {
            if (!(i == 0 && j == 0))
            { // Ignora el elemento central
                if (matriz[f + i][c + j] <= valle)
                { // Si algún vecino es menor o igual, no es valle
                    return 0;
                }
            }
        }
...
} // Esta función comprueba si el elemento en la posición f,c es un elemento valle. Un elemento valle es aquel que es menor que todos sus
vecinos.La función devuelve 1 si el elemento es un valle y 0 en caso contrario.
    // Función que imprime el elemento valle de la posición f,c y sus 8 vecinos adyacentes.
    void
    imprimeValle(int matriz[][NUM_COLUMNAS], int n_filas, int n_columnas, int f, int c)
{
...
// En la función principal, se declara e inicializa la matriz. Luego, se imprime la matriz original, se calcula e imprime la traza de la
// matriz y se buscan e imprimen los elementos valle de la matriz.