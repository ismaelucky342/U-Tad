...
Este documento contiene una breve definición por cada tipo de dato de Redis
 y la solución de los ejercicios originales en forma de comandos `redis-cli`
 con la salida esperada (concisa). Está pensado como apuntes y ejemplos reales.

---

## 1) Claves y Cadenas (Strings)

Definición breve: valores binarios asociados a una clave. Son el tipo más
simple: pueden usarse como texto, números (operaciones atómicas) o blobs.
...
Definición breve: listas ordenadas de strings. Soportan inserción/lectura desde
ambos extremos, indexado y recorte.
...
Definición breve: mapas campo->valor asociados a una clave. Útiles para
objetos/rows pequeños.
...
Definición breve: colecciones no ordenadas de elementos únicos. Operaciones
rápidas para pertenencia, unión, intersección y diferencia.
...
Definición breve: operaciones a nivel de bit sobre strings; útiles para flags
compactos y mapas de bits (bitmaps).
...
Definición breve: operaciones de escaneo no bloqueantes para iterar sobre
colecciones grandes sin cargar todo en memoria.
...
Definición breve: MULTI/EXEC para agrupar comandos; WATCH permite control de
optimistic locking para abortar si hay cambios concurrentes.
...
3) WATCH (ejemplo read-modify-write seguro)
```
WATCH bal:u1
GET bal:u1
...
## 9) Caché y políticas de desalojo
...
Nota: en entornos de producción preferible fijarlo en la configuración del
servidor redis.conf y verificar el comportamiento con INFO memory.

---

## Cómo probar

1) Abre un terminal con el servidor Redis corriendo (por defecto en localhost:6379).
2) Usa `redis-cli` para ejecutar línea a línea los comandos mostrados.
3) Para comandos de expiración con timestamp usa `date +%s` para calcular
	valores de EXPIREAT: p.ej. `EXPIREAT key $(date -d "+1 hour" +%s)`.
4) Para probar WATCH: intenta modificar la clave desde otra sesión mientras
	mantienes la transacción pendiente para ver el abortado.

---

## Notas finales

Estas soluciones están pensadas como ejemplos de referencia rápida para los
apuntes. Si quieres, puedo generar un script `.sh` que ejecute cada bloque
automáticamente y muestre las salidas, o añadir pruebas mínimas con
`redis-cli --eval`.