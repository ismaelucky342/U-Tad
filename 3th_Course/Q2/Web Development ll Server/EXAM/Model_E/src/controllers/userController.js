...
const createUser = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({ message: 'Usuario creado.', user: { id: newUser.id, username: newUser.username, email: newUser.email } });
  } catch (error) {
    return res.status(500).json({ error: 'Error interno.' });
  }
};
const updateUser = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Actualizado.', user: { id: user.id, username: user.username } });
  } catch (error) {
    return res.status(500).json({ error: 'Error interno.' });
  }
};
