...
const login = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      token,
      user: { id: user.id, username: user.username, email: user.email, fullName: user.fullName, isAdmin: user.isAdmin },
    });
  } catch (error) {
...
const createUser = async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      user: { id: newUser.id, username: newUser.username, email: newUser.email, fullName: newUser.fullName, isAdmin: newUser.isAdmin },
    });
  } catch (error) {
    return res.status(500).json({ error: 'Error interno del servidor.', details: error.message });
  }
